...
class Solution {
public:
    TreeNode* buildTree(int preStart, int postStart, int preEnd, vector<int>& preorder, vector<int>& postorder, unordered_map<int, int>& postorderNodeMap;){
        if (preStart > preEnd) return NULL;
        ...
        int leftSubtreeRoot = preorder[preStart + 1];
        int j = postorderNodeMap[leftSubtreeRoot];
    ...
    TreeNode* constructFromPrePost(vector<int>& preorder, vector<int>& postorder) {
        unordered_map<int, int> postorderNodeMap;
        for(int i = 0; i < postorder.size(); i++){
            postorderNodeMap[postorder[i]] = i;
        }

        return buildTree(0, 0, preorder.size() - 1, preorder, postorder, postorderNodeMap);
    }
};
